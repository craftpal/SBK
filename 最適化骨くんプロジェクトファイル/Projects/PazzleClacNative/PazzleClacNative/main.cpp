...
int main(int argc, char *argv[]) {
	...
	switch (argc) {
    case 5:
        weight_x = std::stoi(argv[3]);
        weight_y = std::stoi(argv[4]);
	case 3:
		filepath = argv[2];
	case 2:
	...
	case 1:
		...
		break;
	default:
		std::cout << "引数が無効の為、終了します。" << std::endl;
		return -1;
	}
	if (!jsper.err.empty()) {
    ...
    CCoreRectance *core;
    if (argc == 5) {
        core = new CCoreRectance(lire_targets, lire_parts, weight_x, weight_y);
    } else {
        core = new CCoreRectance(lire_targets, lire_parts);
    }
