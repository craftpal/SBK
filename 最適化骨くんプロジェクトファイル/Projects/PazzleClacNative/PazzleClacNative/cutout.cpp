...
CRectance::CRectance(int x, int y, std::string laminate_id) {
	this->set(x, y);
	this->ranked = 0;
	this->cleared = false;
...
int CListRectance::search_insered(CRectance target, CListRectance parts, int weight_x, int weight_y, int order) {
	//全体ランク付け
	parts.to_rank();
	for (int i = 0, len = parts.lenght(); i < len; i++) {
		...
		if (target.insered(parts.get(i)) && !parts.get(i).Cleared()) {	//対象に部品が入るとき
			parts.Rank(i, weight_x*parts.get(i).Rank().X() + weight_y*parts.get(i).Rank().Y());
		} else {
...
bool CListRectance::del(int index) {
	...
	if (mid) {
        ...
        if (v_rec[index].X() >= v_rec[index - 1].X() && v_rec[index].X() >= v_rec[index + 1].X()) {
            if (v_rec[index + 1].X() <= v_rec[index - 1].X()) {
                v_rec[index - 1].Yadd(v_rec[index].Y());
            } else {
                v_rec[index + 1].Yadd(v_rec[index].Y());
            }
        } else if (v_rec[index].X() >= v_rec[index - 1].X() && v_rec[index].X() <= v_rec[index + 1].X()) {
            v_rec[index].Yadd(v_rec[index + 1].Y());
        } else if (v_rec[index].X() <= v_rec[index - 1].X() && v_rec[index].X() >= v_rec[index + 1].X()) {
            v_rec[index].Yadd(v_rec[index - 1].Y());
        } else {
            throw "削除できません";
...
CFrame::CFrame(CXY topL, CXY lowR, std::string laminate_id) {
	this->topL = topL;
	this->lowR = lowR;
	this->laminate_id = laminate_id;
}
...
void CCoreRectance::init() {
    ...
    for (int i = 0; i < lire_targets.lenght(); i++) {
		target_min.add(i, lire_targets.get(i).X());
	}
#ifdef _DEBUG
	target_min.show_items();
#endif // _DEBUG
	// 最大のXをもつの対象範囲ものからどの部品も入らないものを削除
	bool some_in = false;
    int index_max = target_min.get(target_min.size()-1).index;
    for (int j = 0; j < lire_parts.lenght(); j++) {
        if (lire_parts.get(j).Cleared()) continue;
		if (lire_targets.get(index_max).insered(lire_parts.get(j))) {
			some_in = true;
		}
	}
	if (!some_in) {
		//何も入らない lire_targets のインデックスを削除
		index_del = index_max;	//終了判定用
#ifdef _DEBUG
	...
	Alpha = sl_target_x.get(sl_target_x.size() - 1).index;
#ifdef _DEBUG
    ...
    while (true) {
        ...
        opt = CListRectance::search_insered(lire_targets.get(Alpha), difRecs, weight_x, weight_y, order);
        if (opt < 0) throw opt;
...
#endif // _DEBUG
        // 余白が0以上なら切り出せるため脱出
        if (difAlphaOpt.X() >= 0 && difAlphaOpt.Y() >= 0) break;
        // Xの余白が0以下なら切り出せない
        if (difAlphaOpt.X() < 0 || lire_targets.lenght() <= 1) throw - 1;
...
CCoreRectance::CCoreRectance(CListRectance targets, CListRectance parts)
    : lire_targets(targets), lire_parts(parts) {
    weight_x = 1;
    weight_y = 10;
    finished = false;
    order = 0;
}
CCoreRectance::CCoreRectance(CListRectance targets, CListRectance parts, int weight_x, int weight_y)
    : lire_targets(targets), lire_parts(parts)
...
CFrame CCoreRectance::cutout() {	//挿入した部品の座標を返す
	...
	CFrame result = CFrame(
		...
		lire_parts.get(opt).laminate_id
		);
	//再設定
	//difAlphaOpt.set(
	//	lire_targets.get(Alpha).X() - lire_parts.get(opt).X(),
	...
	while (!finished) {
		try {
			...
			case -1:
				...
				break;
            case -2:
                std::cout << "次の切り出し範囲に行くよ" << std::endl;
#if 0
                //対象範囲から最もX幅が大きいものを取り除く
                int removed = 0;
                for (int i = 0; i < lire_targets.lenght(); i++) {
                    if (lire_targets.get(removed).X() < lire_targets.get(i).X()) {
                        removed = i;
                    }
                }
                lire_targets.del(removed);
                count = 0;
                order = 0;
