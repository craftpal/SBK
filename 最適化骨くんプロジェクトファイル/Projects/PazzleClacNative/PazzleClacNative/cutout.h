...
namespace cutout{
	...
	private:
		...
		bool cleared;
	public:
		std::string laminate_id;
		CRectance();
		CRectance(int x, int y);
		CRectance(int x, int y, std::string laminate_id);
	...
	private:
		...
		void to_rank();
	public:
		static int search_insered(CRectance target, CListRectance parts, int weight_x, int weight_y, int order);
	...
	public:
		...
		CFrame(CXY topL, CXY lowR, std::string laminate_id);
		void show();
		//左 X からの相対座標( Y は絶対)
		void show(int x);
	};
	...
	public:
        ...
        CCoreRectance(CListRectance targets, CListRectance parts, int weight_x, int weight_y);
		CFrame cutout();
